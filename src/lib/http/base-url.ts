// ============================================================
// Resolve the canonical public base URL of this deployment from an
// incoming request. Originally lived in
// src/app/api/account/invitations/route.ts (invite links); extracted
// here so the Google OAuth routes (which need the exact same
// "what's my public URL on Vercel vs a proxied VPS" resolution to
// build a correct redirect_uri) share one implementation instead of
// drifting.
//
...
//   receive a URL pointing at their site — for Google OAuth
//   specifically, that would mean the redirect_uri (and thus where
//   the authorization code lands) is attacker-controlled, so this
//   check matters here even more than for invite links.
//
...
export function getBaseUrl(request: Request, logPrefix = "[getBaseUrl]"): string {
